...
cmd_proxy() {
    ...
    cmd="$proxy -E 10000 -n $tosend isock -p PULL -a connect -e tcp://127.0.0.1:$port1 osock -p PUSH -a bind -e tcp://127.0.0.1:$port2"
    echo $cmd
    $cmd 2>&1 > $logf
}
...
cmd_sender () {
    ...
    cmd="$sender -E 10000 -d 0 -m 10 -p PUSH -a bind -e tcp://127.0.0.1:$port1 -B 1000 -n $tosend -N 1000 uniform -t10"
    echo $cmd
    $cmd 2>&1 > $logf
}
...
cmd_reciever () {
    ...
    cmd="$recver -n $tosend -p PULL -a connect -e tcp://127.0.0.1:$port2"
    echo $cmd
    $cmd 2>&1 > $logf
}    
