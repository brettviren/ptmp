// note, this needs ptmp-tcs plugin to be available
...
local zipper = ptmp.zipper("zipper",
                           ...
                           osocket = ptmp.socket('bind','push', "inproc://zipped"),
                           cfg = {sync_time:sync_time, tardy_policy:"drop"});

local tcfinder = ptmp.nodeconfig("tcfinder","tcfinder",
                                 isocket = ptmp.socket('connect','pull', "inproc://zipped"),
                                 osocket = ptmp.socket('bind','pub', output, hwm=pubsub_hwm),
                                 cfg = {method: "pdune-adjacency"});

{
    plugins: ["ptmp-tcs"],
    proxies: windows + [zipper, tcfinder],
}
